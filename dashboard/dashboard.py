...
with tab2:
    ...
    if visualization_option == "Weekday":
        sns.lineplot(data=hourly_df[hourly_df["day_type"] == "Weekday"], 
                     x="hr", y="total_rentals", marker="o", color="blue", ax=ax)
        ax.set_title("Penyewaan Sepeda pada Hari Kerja (Weekday)")
    elif visualization_option == "Weekend":
        sns.lineplot(data=hourly_df[hourly_df["day_type"] == "Weekend"], 
                     x="hr", y="total_rentals", marker="o", color="orange", ax=ax)
        ax.set_title("Penyewaan Sepeda pada Akhir Pekan (Weekend)")
    else:
        sns.lineplot(data=hourly_df, x="hr", y="total_rentals", hue="day_type",
                     marker="o", palette=["blue", "orange"], ax=ax)
        ax.legend(title="Tipe Hari")
        ax.set_title("Penyewaan Sepeda Berdasarkan Waktu")
    ...
    if visualization_option == "Weekday":
        ...
        - Penyewaan meningkat pada pagi (07:00-09:00) & sore (17:00-19:00), menunjukkan penggunaan untuk perjalanan kerja/sekolah.""")
    elif visualization_option == "Weekend":
        st.markdown("""**📌 Kesimpulan**:  
        - Puncak penyewaan terjadi siang hingga sore (11:00-17:00), menandakan penggunaan untuk rekreasi/santai.""")
    else:
        st.markdown("""**📌 Kesimpulan**:  
